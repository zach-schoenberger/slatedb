...
impl SlateDbWalWriter {
    ...
    pub(crate) async fn start_new(
        ...
        // The WAL buffer holds a second in-memory copy of unflushed writes
        // (alongside the memtable), so its bytes are charged against the DB's
        // shared write-buffer budget. Each WAL buffer's permit is released when
        // it is flushed to remote storage and dropped, freeing budget for new
        // writes just like a memtable freed after its L0 flush.
...
impl SlateDbWalWriterInner {
    ...
    fn freeze_current_wal(&mut self) {
        ...
        let current_wal = std::mem::replace(
            &mut self.current_wal,
            WalBuffer::new(&self.write_buffer_manager),
        );
        self.flush_epoch += 1;
        self.immutable_wals
